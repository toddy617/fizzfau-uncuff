--Skrypt By Ruski, Contact Information @Ruski#0001 on Discord, Made For PlanetaRAnimAresztujeP
...
local SekcjaAnimacji			= 'mp_arresting'	-- Sekcja Katalogu Animcji
local AnimAresztuje 			= 'a_uncuff'	-- Animacja Aresztujacego
local AnimAresztowany			= 'b_uncuff'	-- Animacja Aresztowanego
...
RegisterNetEvent('fizzfau-uncuff:uncuff1')
AddEventHandler('fizzfau-uncuff:uncuff1', function(target)
	Aresztowany = true
...
RegisterNetEvent('fizzfau-uncuff:uncuff2')
AddEventHandler('fizzfau-uncuff:uncuff2', function()
	local playerPed = GetPlayerPed(-1)
...
-- 				TriggerServerEvent('fizzfau-uncuff:handcuff', GetPlayerServerId(closestPlayer))									-- Rozpoczyna Funkcje na Animacje (Cala Funkcja jest Powyzej^^^)
